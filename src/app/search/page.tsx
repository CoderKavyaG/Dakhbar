...
export default async function SearchPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return <main className="paper-shell search-page">
    <header className="compact-header"><Wordmark /><span>Search / Explore</span></header>
    ...
    {query && <section className="search-results">
      ...
      {results.intent === 'navigational' && results.entityMatches[0] && <div className="entity-match"><p className="kicker">Matched entity</p><h3>{results.entityMatches[0].name}</h3><p>{results.entityMatches[0].type}</p></div>}
      <div className="result-list">{results.stories.map(result => <article key={result.id}><p className="kicker">Updated {result.updated_at.toLocaleString('en-IN')}</p><h3><Link href={'/stories/' + result.id}>{result.title}</Link></h3></article>)}</div>
