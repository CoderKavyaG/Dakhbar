...
export default async function FrontPage(){
 ...
 return <main className="paper-shell"><div className="edition-intro"><p>{date}</p><Badge>{stories.length} selected stories</Badge></div><section className="lead-stage"><article className="lead-copy"><div className="lead-meta">{lead.entities.slice(0,3).map(e=><Link key={e.entity_id} href={'/search?q='+encodeURIComponent(e.entity.name)}>{e.entity.name}</Link>)}</div><h1><a href={destination.href}>{lead.title}</a></h1><p className="lead-dek">{dek.kind==='domain'?'Reporting from ':''}{dek.text}</p><div className="lead-actions"><Button asChild><a href={destination.href}>{destination.external?'Read original':'Follow the story'} ↗</a></Button>{count>1&&<span>{count} reporting sources</span>}</div></article><TodayEdition title={lead.title} href={destination.href} external={destination.external} image={image} date={date}/></section>
