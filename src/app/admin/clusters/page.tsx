...
export default async function ClusterReviewPage() {
  ...
  return <main className="admin-shell">
    <header className="admin-header"><div><p className="kicker">Dअख़बार operations</p><h1>Cluster review</h1></div><nav><Link href="/admin">Ingestion</Link><Link href="/">Front Page</Link></nav></header>
    ...
    <div className="cluster-list">{data.stories.map(story => <article className={story.status === 'review_needed' ? 'cluster-card cluster-review' : 'cluster-card'} key={story.id}>
      <header><div><p className="kicker">{story.status.replace('_', ' ')} · score {story.significance_score.toFixed(3)}</p><h2>{story.title}</h2></div><div className="entity-row">{story.entities.map(item => <span key={item.entity_id}>{item.entity.name}</span>)}</div></header>
