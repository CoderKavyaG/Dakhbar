...
export default async function StoryPage({params}:{params:Promise<{id:string}>}){
 ...
 return <main className="paper-shell story-page"><nav className="breadcrumbs" aria-label="Breadcrumb"><Link href="/">Today</Link><span>/</span><span>Story & sources</span></nav><article className="story-detail"><header className="story-detail-header"><div><div className="story-context">{story.entities.map(e=><EntityFollowControl key={e.entity_id} entity={{id:e.entity_id,name:e.entity.name}} returnTo={topicPath(e.entity)}/>)}</div><h1>{story.title}</h1><p className="detail-dek">{dek.kind==='domain'?'Reporting from ':''}{dek.text}</p><div className="detail-actions"><Badge>{countIndependentSources(documents)} reporting sources</Badge><FollowToggle entityIds={story.entities.map(e=>e.entity_id)} returnTo={'/stories/'+story.id} followLabel="Follow the story" followingLabel="Following this story" variant="outline"/></div></div><StoryImage src={image} alt="" className="story-detail-image"/></header>
